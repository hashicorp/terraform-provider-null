...
func dataSourceSensitive() *schema.Resource {
	return &schema.Resource{
		...
		Description: `The ` + "`null_data_source_sensitive`" + ` data source implements the standard data source lifecycle but does not
...
`,
		...
		Schema: map[string]*schema.Schema{
			"inputs": {
				Description: "A map of arbitrary strings that is copied into the `outputs` attribute, and accessible directly for interpolation. Its value is marked as Sensitive and will be masked in the UI.",
				Type:        schema.TypeMap,
				Sensitive:   true,
				Optional:    true,
			},
			"outputs": {
				Description: "After the data source is \"read\", a copy of the `inputs` map. Its value is marked as Sensitive and will be masked in the UI.",
				Type:        schema.TypeMap,
				Sensitive:   true,
				Computed:    true,
			},
...
func dataSourceSensitiveRead(d *schema.ResourceData, meta interface{}) error {
